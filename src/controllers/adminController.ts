...
export const adminLogin = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const isPasswordValid = await compare(password, admin.get().password);
    if (!isPasswordValid) {
...
export const getDefaultPassword = async (req: Request, res: Response) => {
  try {
    ...
    const college = (
      await College.findOne({ where: { collegeId: admin.collegeId } })
    ).get();
    // console.log(college);
    // res.end(college.defaultPassword);
    // console.log(college.defaultPassword);
    return res.status(200).json({ defaultPassword: college.defaultStudentPassword });
  } catch (error) {
