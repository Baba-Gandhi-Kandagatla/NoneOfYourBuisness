...
import { Q1, getFeedback, genNextQuestion, genNextCodeQuestion, getFinalFeedback,getCodeFeedback, reframeQuestion, genEvalMatrix } from "../Api-helper/helper.js";
...
export const startInterview = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const firstQuestion = await Q1(resume, interview.subject, interview.topic);
...
export const getNext = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!lastExchange.response) {
      ...
      {
        const{feedback, marks}  = await getCodeFeedback(lastExchange.question, lastExchange.code, studentResponse);
        lastExchange.feedback = feedback;
        lastExchange.response = studentResponse;
        lastExchange.marks = marks;
      }
      else{
        const{feedback, marks}  = await getFeedback(lastExchange.question, studentResponse);
        lastExchange.feedback = feedback;
        lastExchange.response = studentResponse;
        lastExchange.marks = marks;
      }
    ...
    if(interviewExchanges.length >= interview.no_of_questions) {
      [nextQuestion, code ]= await genNextCodeQuestion(resume, interview.subject, interview.topic, interviewExchanges);
    }
    else{
      nextQuestion = await genNextQuestion(resume, interview.subject, interview.topic, interviewExchanges);
    }
...
export const reframeLastQuestion = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const reframedQuestion = await reframeQuestion(lastExchange.question);
...
export const submitInterview = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const resume = await fetchResumeContext(student_roll);
    const finalFeedback = await getFinalFeedback(resume, interview.subject, interview.topic, interviewExchanges);
    ...
    const {problem_solving,code_quality,debugging} = await genEvalMatrix(resume, interview.subject, interview.topic, interviewExchanges);
