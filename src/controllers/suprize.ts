...
const findUserByRollNumber = async (rollNumber: string) => {
  return await Student.findOne({ where: { roll_number: rollNumber } }) ||
...
export const verifyUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).json({ rollNumber: user.dataValues.roll_number, role: user instanceof Student ? 'student' : 'admin', username: user.dataValues.username });
  } catch (error) {
